...
## Output
Two files saved **in the same folder as the transcript**, named:
- `สรุปการประชุม-<topic>.md`
- `สรุปการประชุม-<topic>.docx`
...
## Steps
1. Read the transcript fully (page through if large).
2. Draft the report and **Write** the `.md` file (structure above).
3. Generate the `.docx` from that `.md` by running the bundled script:
   ```bash
   python3 "<skill_dir>/scripts/gen_docx.py" "<path>/สรุปการประชุม-<topic>.md"
   ```
...
4. Tell the user both file paths, and list any speaker names/roles you were unsure about so they can confirm.
